...
# --- MODIFIED: The generate_image function is now smarter ---
def generate_image(pipe, prompt, negative_prompt, steps, guidance_scale, model_name):
    """
    Generates an image, intelligently adjusting parameters for special models.
    """
    try:
        # Start with a base dictionary for arguments
        kwargs = {
            "prompt": prompt
        }
        
        # Case 1: For the custom Japanese model, it only accepts the prompt
        if "japanese-stable-diffusion-xl" in model_name:
            print(">>> Japanese model detected, using prompt-only generation.")
            pass
            # We pass only the prompt, as per the official example.
            # The kwargs dictionary is already correctly set up.
        
        # Case 2: For Turbo models, use optimized parameters
        elif "turbo" in model_name:
            print(">>> Turbo model detected, using optimized parameters.")
            kwargs["negative_prompt"] = negative_prompt
            kwargs["num_inference_steps"] = 1
            kwargs["guidance_scale"] = 0.0
        
        # Default case for standard models (SD3, SDXL, etc.)
        else:
            print(">>> Standard model detected, using all parameters.")
            kwargs["negative_prompt"] = negative_prompt
            kwargs["num_inference_steps"] = steps
            kwargs["guidance_scale"] = guidance_scale
        
        # Add max_sequence_length for all SD3 models
        if "stable-diffusion-3.5-medium" in model_name:
            kwargs["max_sequence_length"] = 512
            
        print(f">>> Calling pipeline with arguments: {list(kwargs.keys())}")
        ...
        # To see the full error details for debugging, show the traceback
